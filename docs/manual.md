...
| `lore sync resign [--apply] [--replace-foreign-key]` | sign this machine's own unsigned ops with the current `LORE_SYNC_HMAC_KEY` — for a log that grew before the key existed. Dry run by default; reports how many ops are unsigned, signed with the current key, signed with a different key, or malformed, and how many would be resigned. `--apply` writes, after backing up `state.db` next to itself with a timestamp. Never signs another machine's ops. An op signed under a different key is reported and left alone unless `--replace-foreign-key` is also given. |
| `lore sync seed [--apply]` | back-fill the op log for portable state (memory, file maps, beliefs, skills) this store held before the log ever ran — the op log records CHANGES since it started, not a snapshot of what already existed. Dry run by default; reports, per class, how many ops would be appended. `--apply` writes one op per piece of state the log cannot already reproduce, after backing up `state.db`. Idempotent: a second `--apply` appends nothing. Never seeds `pending` (a human-review queue, not curated truth) and never touches machine memory. Respects `LORE_SYNC_CLASSES`. |
...
If export refuses an op as unsigned — a log that grew before `LORE_SYNC_HMAC_KEY` was ever set — run `lore sync resign --apply` first. It only signs ops this machine authored, so an unsigned op that belongs to another machine is reported and left standing; export will still refuse that one.

If a store curated memory, beliefs, skills, or a file map for a while before the sync log (or the key) existed at all, `sync resign` alone is not enough: it can only sign ops that are already in the log, and pre-log state has no op to sign. Run `lore sync seed --apply` after resigning — it appends exactly the ops the log is missing, one per piece of state, so export (and a fresh peer's bootstrap) ship the store as it stands today rather than only the fragment written after sync was switched on.
